...
class GameBlocks:
    ...
    def load_sprites(self) -> list:
        files = [item for item  in pathlib.Path('./assets/img/tiles/blocks').rglob('*.png') if item.is_file()]
...
class ParticleEffect:
    def __init__(self, screen, pos, color, max_particles=50, particle_speed=8, particle_size=1):
        self.screen = screen
        self.pos = pos
        self.color = color
    ...
    def create_particles(self):
        for i in range(self.max_particles):
            particle = {
                'pos': [self.pos[0] + random.uniform(-35, 35), self.pos[1] + random.uniform(-35, 35)],
                'speed': [random.uniform(-self.particle_speed, self.particle_speed), random.uniform(-self.particle_speed, self.particle_speed)],
                'color': self.color,
                'life': random.uniform(0.2, .5)
            }
    ...
    def draw(self):
        for particle in self.particles:
            pg.draw.circle(self.screen, particle['color'], (int(particle['pos'][0]), int(particle['pos'][1])), self.particle_size)